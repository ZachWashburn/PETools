...
#define MAKE_SYSCALL_FUNCTION_SPECIALIZED_TEMPLATE(CallConv)											\
	...
	{																									\
		...
		_windows_syscall_t(void* pfnFunction){}						                                    \
		_windows_syscall_t(const char* szFuncName, const char* szModule)						        \
		{																								\
		}																								\
		_windows_syscall_t(unsigned long ulHash, const char* szModule)							        \
		{																								\
		}																								\
...
#define WINDOWS_SYSCALL(Name, Type, Code) _windows_syscall_t<type> _##Name##(Code);
#define GLOBAL_WINDOWS_SYSCALL(Name, Type, Code) inline _windows_syscall_t<type> g_##Name##(Code);
